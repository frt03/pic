...
from pic.gym.reward_shaping.dynamic_mjc import MJCModel
import numpy as np
import random
...
def point_maze(maze_str):
    ...
    asset.texture(
        name="skybox", type="skybox", builtin="gradient", rgb1=".4 .6 .8", rgb2="0 0 0",
...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        if self.reward_type == 'negative_sparse':
            reward = 0.0 if np.linalg.norm(ob[0:2] - self._target) <= self.distance_threshold else -1.0
            assert reward < 0.001
        elif self.reward_type == 'densel1':
            reward = -self.coefficent * np.linalg.norm(ob[0:2] - self._target, ord=1)
            assert reward < 0.0
        elif self.reward_type == 'densel2':
            reward = -self.coefficent * np.linalg.norm(ob[0:2] - self._target, ord=2)
            assert reward < 0.0
        elif self.reward_type == 'frac':
            d = np.linalg.norm(ob[0:2] - self._target, ord=2)
