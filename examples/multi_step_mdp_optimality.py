...
def main():
    ...
    parser.add_argument("--mi_population_size", type=int, default=100, help="population for finite difference")
    parser.add_argument("--mi_coefficient", type=float, default=1.0, help="coefficient for MI")
    parser.add_argument("--mi_scale", type=float, default=1.0, help="scale parameter for finite difference")
    ...
    output_dir = os.path.join(
        args.output,
        'optimal_multi_step_es_horizon{}'.format(args.horizon),
        'ps{}_pm{}_pop{}_ep{}_seed-{}'.format(args.prior_sigma, args.prior_mean, args.population_size, args.episodes_per_param, args.seed),
        timestamp
        )
    ...
    for itr in range(args.iterations):
        ...
        p_theta = st.multivariate_normal(mean=prior_mu, cov=args.prior_sigma)
        ...
        mutual_info = trial.value
        temperature = trial.params['temperature']
        p_o1 = np.exp((all_scores-r_max)/temperature).mean()
        p_o1_ts = np.exp((all_scores_per_param-r_max)/temperature).mean(axis=1)
        marginal = -p_o1*np.log(p_o1) - (1-p_o1)*np.log(1-p_o1)
        conditional = np.mean(-p_o1_ts*np.log(p_o1_ts + 1e-12) - (1-p_o1_ts)*np.log(1-p_o1_ts + 1e-12))
